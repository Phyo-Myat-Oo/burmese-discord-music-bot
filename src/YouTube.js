...
class YouTube {
    ...
    static getYtDlpOptions(extraOptions = {}) {
        const baseOptions = {
            ...
            retries: 3,
            fragmentRetries: 3,
            // Passing an absolute Windows path here breaks youtube-dl-exec's
            // shell command when the path contains spaces (for example,
            // C:\Program Files\nodejs\node.exe). yt-dlp can resolve Node from
            // PATH, which is portable across local Windows and the Linux VPS.
            jsRuntimes: 'node',
            ...extraOptions
        };

        // Authentication priority: PO token > browser cookies > cookie file.
        // With no authentication configured, allow yt-dlp to select its current
        // working client set. Forcing the iOS client can hide audio formats as
        // YouTube changes client capabilities.
