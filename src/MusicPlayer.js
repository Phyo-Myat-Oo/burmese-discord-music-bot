...
const { PhyuPlaybackProvider } = require('./providers/PhyuPlaybackProvider');
...
const phyuPlaybackProvider = new PhyuPlaybackProvider();
...
class MusicPlayer {
    constructor(guild, textChannel, voiceChannel) {
        ...
        this.preloadWindowPromise = null;
    ...
    async enforceCacheLimit() {
        ...
        while (this.downloadedFiles.size > MAX_CACHED_TRACKS) {
            const oldest = Array.from(this.downloadedFiles)
                .find(filepath => !protectedFiles.has(filepath));
            if (!oldest) break;
            await this.deleteDownloadedFile(oldest);
        }
    }
    ...
    async downloadTrack(track, streamUrl, streamInfo) {
        ...
        try {
            // A non-empty output file is not necessarily complete while yt-dlp
            // or FFmpeg still owns it. Wait for the active writer before
            // treating the path as a valid fallback cache entry.
            if (this.downloadingFiles.has(filepath)) {
                for (let i = 0; i < 60; i++) {
                    await new Promise(resolve => setTimeout(resolve, 1000));

                    if (!this.downloadingFiles.has(filepath) && fsSync.existsSync(filepath)) {
        ...
        try {
            ...
            if (!streamInfo) {
                ...
                switch (this.currentTrack.platform) {
                    ...
                    case 'phyu':
                        streamInfo = await phyuPlaybackProvider.resolve(this.currentTrack);
                        break;
            ...
            const expectedCachePath = this.getTrackCachePath(this.currentTrack);
            
            if (
                this.currentDownloadedFile === expectedCachePath &&
               ...
               downloadedFile = this.currentDownloadedFile;
                ...
                const filepath = expectedCachePath;
    ...
    async handleTrackEnd(reason = 'idle') {
        ...
        try {
            ...
            // Keep completed tracks in the rolling cache. The oldest unprotected
            // file is removed automatically when the cache exceeds ten tracks.
    ...
    schedulePreloadWindow() {
        ...
        this.preloadWindowPromise = (async () => {
            ...
            } while (this.preloadWindowPending);
        })().catch(error => {
            console.error(`Preload window failed: ${error.message}`);
        }).finally(() => {
            this.preloadWindowPromise = null;
        });
        ...
        try {
            ...
            switch (track.platform) {
                ...
                case 'direct':
                    ...
                    break;
                case 'phyu':
                    streamInfo = await phyuPlaybackProvider.resolve(track);
                    break;
            }
